...
public class PPMainBoardforNu extends MainBoard {
		...
		try {
	        ...
	        while(true)
	        {
	         ...
	         //判断本机是不是房主
                ...
                local=addr.getHostAddress().toString(); 
	        //发送准备好信息
                /*if(owner1 != null && follow != null){*/
	         ...
	         System.out.println("********************************");
                	...
                	if(local.equals(cb.owner2)){
                        NetTool.sendUDPBroadCast(cb.follow2,"ready, ");
                        gameState="ready";
        	         }else{
        	        	 NetTool.sendUDPBroadCast(cb.owner2,"ready, ");
        	                gameState="ready";
        	         }
                }*/
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        else if(e.getSource()==back)
        {
        	...
        	if(owner1 == null && follow == null){
        	if(local.equals(cb.owner2)){
        	NetTool.sendUDPBroadCast(cb.follow2,"back"+", ");	
        	logger.info("玩家选择悔棋");
        	}else{
        		NetTool.sendUDPBroadCast(cb.owner2,"back"+", ");	
            	logger.info("玩家选择悔棋");
        	}
        	}else{
        		if(local.equals(owner1)){
                	NetTool.sendUDPBroadCast(follow,"back"+", ");	
                	logger.info("玩家选择悔棋");
                	}else{
                		NetTool.sendUDPBroadCast(owner1,"back"+", ");	
                    	logger.info("玩家选择悔棋");
                	}
        	}
        }
        ...
        else if(e.getSource()==send)
        {
        	if(!talkField.getText().isEmpty()&&!talkField.getText().equals("不能为空"))
        	{
        		...
        		try {
					local =  InetAddress.getLocalHost().getHostAddress();
					System.out.println("local:"+local+"owner:"+owner1+"follow:"+follow);
					if(owner1 != null && follow != null){
					if(local.equals(owner1)){
		        		NetTool.sendUDPBroadCast(follow,"enemy"+","+msg);
		        		}else{
		        			NetTool.sendUDPBroadCast(owner1,"enemy"+","+msg);
		            	}
					}else{
						if(local.equals(cb.owner2)){
			        		NetTool.sendUDPBroadCast(cb.follow2,"enemy"+","+msg);
			        		}else{
			        			NetTool.sendUDPBroadCast(cb.owner2,"enemy"+","+msg);
			            	}
					}
				} catch (UnknownHostException e1) {
					// TODO Auto-generated catch block
        ...
        else if(e.getSource()==exitGame)
        {
			...
			if(owner1 != null && follow != null){
			if(local.equals(owner1)){
        		NetTool.sendUDPBroadCast(follow,msg+", ");
        		}else{
        			NetTool.sendUDPBroadCast(owner1,msg+", ");
            	}
			}else{
				if(local.equals(cb.owner2)){
	        		NetTool.sendUDPBroadCast(cb.follow2,msg+", ");
	        		}else{
	        			NetTool.sendUDPBroadCast(cb.owner2,msg+", ");
	            	}
			}
            dispose();
            new SelectMenu();
                ...
                try
                {
                    ...
                    while(true)
                    {
                        ...
                        if(data.isEmpty())
                        {
                            ...
                            {
                            	talkArea.append("对手："+new String(msg[1].getBytes(),"UTF-8")+"\n");
                            ...
                            else if(msg[0].equals("back"))
                            {
                            	...
                            	if(n==JOptionPane.YES_OPTION)
                            	{
                            		...
                            		try {
                    					...
                    					if(owner1 != null && follow !=null){
                    					if(local.equals(owner1)){
                                    		NetTool.sendUDPBroadCast(follow, "canBack"+", ");
                                    		}else{
                                    			NetTool.sendUDPBroadCast(owner1, "canBack"+", ");
                                    		}
                    					}else{
                    						if(local.equals(cb.owner2)){
                                        		NetTool.sendUDPBroadCast(cb.follow2, "canBack"+", ");
                                        		}else{
                                        			NetTool.sendUDPBroadCast(cb.owner2, "canBack"+", ");
                                        		}
                    					}
                    				} catch (UnknownHostException e1) {
                    					// TODO Auto-generated catch block
                					...
                					if(owner1 != null && follow != null){
                            		if(local.equals(owner1)){
                                		NetTool.sendUDPBroadCast(follow, "noBack"+", ");
                                		}else{
                                			NetTool.sendUDPBroadCast(owner1, "noBack"+", ");
                                		}
                            	}else{
                            		if(local.equals(cb.owner2)){
                                		NetTool.sendUDPBroadCast(cb.follow2, "noBack"+", ");
                                		}else{
                                			NetTool.sendUDPBroadCast(cb.owner2, "noBack"+", ");
                                		}
								}
                            	}
                            }
                            ...
                            {
                            	JOptionPane.showConfirmDialog(cb, "对方退出游戏");
                            	dispose();
                            }
